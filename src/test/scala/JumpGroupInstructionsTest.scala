...
class JumpGroupInstructionsTest extends AnyWordSpec {
  ...
  "JMP" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x44))
  ...
  "JFC" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if C is false" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x40))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
  ...
  "JFZ" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if Z is false" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x48))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
  ...
  "JFS" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if S is false" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x50))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
  ...
  "JFP" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if P is false" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x58))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
  ...
  "JC" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if C is true" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x60))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
      cpu.state = cpu.state.copy(flags = cpu.state.flags.copy(carry = true))
      val state = cpu.step
  ...
  "JZ" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if Z is true" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x68))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
      cpu.state = cpu.state.copy(flags = cpu.state.flags.copy(zero = true))
      val state = cpu.step
  ...
  "JS" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if S is true" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x70))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
      cpu.state = cpu.state.copy(flags = cpu.state.flags.copy(sign = true))
      val state = cpu.step
  ...
  "JP" should {
    "set the PC (unconditionally jump) to memory address B3...B3B2...B2 if P is true" in {
      val ram = Memory(new Array[UByte](Memory.MAX_MEMORY)).writeByte(0x0000, UByte(0x78))
        .writeByte(0x0001, UByte(0x04)).writeByte(0x0002, UByte(0x02))
      val cpu = new CPU(ram)
      cpu.state = cpu.state.copy(flags = cpu.state.flags.copy(parity = true))
      val state = cpu.step
